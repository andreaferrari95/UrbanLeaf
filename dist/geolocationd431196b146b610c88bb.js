...
function getLocationAndRequest() {
  getCurrentPosition()
    ...
    .then((cityScores) => updateCityInfo(cityScores))
    .then(() => getWeatherData(cityScores.lat, cityScores.lon))
...
function getCityDetails(cityData) {
  ...
  return {
    lat: _.get(cityData, "location.latlon.latitude"),
    lon: _.get(cityData, "location.latlon.longitude"),
    cityNameForCalling: cityName.toLowerCase(),
  };
}

function getCityScores(cityDetails) {
  return axios
    .get(
      `https://api.teleport.org/api/urban_areas/slug:${cityDetails.cityNameForCalling}/scores/`
    )
    .then((response) => {
...
function getLocationAndRequest() {
  getCurrentPosition()
    ...
    .then((response) => {
      ...
      // Get city details using the href
    ...
    .then(() => {
      ...
      // Get weather data using city's latitude and longitude
...
async function getWeatherData(latitude, longitude) {
  const url = `http://api.airvisual.com/v2/nearest_city?lat=${latitude}&lon=${longitude}&key=${"4fb35cd8-01d9-4068-a090-0634e06a0e4a"}`;
  const response = await axios__WEBPACK_IMPORTED_MODULE_1__["default"].get(url);
...
function updateWeatherInfo(weatherData) {
  ...
  } = lodash__WEBPACK_IMPORTED_MODULE_0___default().get(weatherData, "data.current.weather");
...
function updateScoreCircleAnimation(roundedScores) {
  ...
  let counter = 0;
  const intervalDuration = Math.floor(animationDuration / roundedScores);
...
/******/ })()
;
//# sourceMappingURL=geolocationd431196b146b610c88bb.js.map